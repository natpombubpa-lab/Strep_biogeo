...
```{r}
tax.bac <- as(tax_table(physeq.prune.sample.noneg.notaxa4),"matrix")
head(tax.bac)
...
bac.3 <- phyloseq(sample_data(physeq.prune.sample.noneg.notaxa4),otu_table(physeq.prune.sample.noneg.notaxa4),TAX.bac)
bac.3
```
...
## EMPO2
```{r}
bac.abund <- merge_samples(glom.bac, "empo_2")
sample_data(bac.abund)$empo_2 <- factor(sample_names(bac.abund))
...
taxcom_layer_pl = ggplot(data = data_glom.bac, mapping = aes_string(x = "empo_2" ,y = "Abundance", fill = "Species")) + 
  geom_bar(stat="identity", position="fill") + 
  ggtitle("Streptomyces Taxonomic Composition by EMPO level 2")+
...
```{r}
png("./Figures/Taxa_plots/Streptomyces_Taxa_by_EMPO2.png", units="in", width = 10, height = 8, res = 600 )
taxcom_layer_pl + theme(legend.position="bottom") + guides(fill=guide_legend(nrow=8)) #+ theme(axis.text.x = element_text(angle = 45, hjust = 1))
dev.off()
```
